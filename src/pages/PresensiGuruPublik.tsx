...
import { GraduationCap, ChevronLeft, ChevronRight, Search, RefreshCw, Calendar, ArrowLeft } from 'lucide-react';
...
export default function PresensiGuruPublik() {
  ...
  const loadData = async (p = 1) => {
    ...
    try {
      ...
      const r: any = await api.get(`/api/public/presensi/guru?${params}`);
      setRows(r.data || []);
      setTotal(r.total || 0);
      setPage(r.page || 1);
      setPages(r.totalPages || 1);
    } catch (err: any) {
      toast.error(err?.message || 'Gagal memuat data presensi guru');
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="flex-grow pt-28 pb-16 px-4 md:px-20 max-w-screen-2xl mx-auto w-full">
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-on-surface">Presensi Guru</h1>
              <p className="text-on-surface-variant mt-1">Data kehadiran guru secara real-time</p>
            </div>
          </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-outline-variant">
                    {['No','Nama Guru','Tanggal','Jam Datang','Jam Pulang','Durasi','Status'].map(h => (
                    ...
                    rows.map(row => (
                      <tr key={row.id} className="hover:bg-surface-container-low/50 transition-colors">
                        ...
                        <td className="px-5 py-4 font-semibold text-primary">
                          {row.nama}
                          <div className="text-xs text-on-surface-variant font-normal mt-0.5">NIP: {row.nip}</div>
                        </td>
                        ...
                        <td className="px-5 py-4">
                          {row.autoCheckout ? (
                            <span className="text-[11px] font-bold px-2 py-1 bg-tertiary-fixed text-on-tertiary-fixed rounded-full">
                              Auto
                            </span>
                          ) : row.waktuPulang ? (
                            <span className="text-[11px] font-bold px-2 py-1 bg-primary-fixed text-primary rounded-full">
                              Manual
                            </span>
                          ) : (
                            <span className="text-[11px] font-bold px-2 py-1 bg-error-container text-error rounded-full">
                              Belum Pulang
                            </span>
                          )}
                        </td>
